...
class Myo(MyoRaw):
    ...
    def add_raw_arm_handler(self, h):
        self.arm_handlers.append(h)
    def add_raw_imu_handler(self, h):
        self.imu_handlers.append(h)
    def add_raw_emg_handler(self, h):
    ...
    def on_raw_pose(self, pose):
        for h in self.pose_handlers:
            ...
            if pose == 2:
                print("")
                #os.system("rosservice call /gazebo/set_model_state '{model_state: { model_name: coke_can, pose: { position: { x: 0.3, y: 0.2 ,z: 0 }, orientation: {x: 0, y: 0.491983115673, z: 0, w: 0.870604813099 } }, twist: { linear: {x: 0.0 , y: 0 ,z: 0 } , angular: { x: 0.0 , y: 0 , z: 0.0 } } , reference_frame: world } }'")
            elif pose == 3:
                print("")
                #os.system("rosservice call /gazebo/set_model_state '{model_state: { model_name: rrbot, pose: { position: { x: 1, y: 1 ,z: 10 }, orientation: {x: 0, y: 0.491983115673, z: 0, w: 0.870604813099 } }, twist: { linear: {x: 0.0 , y: 0 ,z: 0 } , angular: { x: 0.0 , y: 0 , z: 0.0 } } , reference_frame: world } }'")	

if __name__ == '__main__':
    import subprocess
    m = Myo(NNClassifier(), sys.argv[1] if len(sys.argv) >= 2 else None)
    
    m.add_raw_arm_handler(print)
